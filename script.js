...
class TestimonialCarousel {
    ...
    initialize() {
        // Duplicate testimonials for seamless loop (exactly double the set)
        ...
        // Now the track has 2x the original cards; we'll animate -50% to loop perfectly.
        // The CSS uses translateX(-50%) so it works if the total width is exactly 2x the original set.
...
// ============================================
// CHECKOUT PAYMENT FLOW (unchanged except prices)
// ============================================
...
class Navigation {
    ...
    initialize() {
        ...
        // Mobile menu toggle (existing logic kept)
    }
}
...
class VisitorCounter {
    ...
    simulate() {
        // Random number between 35 and 70, changes every few seconds
        const update = () => {
            const count = Math.floor(Math.random() * 36) + 35; // 35-70
...
class SocialProofToasts {
    constructor() {
        this.names = ['Sarah', 'Mike', 'Jessica', 'David', 'Emily', 'Chris', 'Laura', 'Tom', 'Rachel', 'Kevin'];
        this.locations = ['London', 'Manchester', 'Birmingham', 'Liverpool', 'Glasgow', 'Dublin'];
        this.interval = null;
        this.start();
    }
    start() {
    ...
    showRandomToast() {
        ...
        // Auto-remove after animation (4s total)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Scarcity triggers (only on homepage where elements exist)
    if (document.getElementById('visitorCount')) {
        window.visitorCounter = new VisitorCounter();
    }
    if (document.querySelector('.ticket-card')) {  // any ticket card indicates homepage
        window.socialProof = new SocialProofToasts();
    }
    
    // Notification styles (already added in original script, keeping)
